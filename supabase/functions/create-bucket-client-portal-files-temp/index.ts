...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'client-portal-files',
        name: 'client-portal-files',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for client-portal-files" ON storage.objects FOR SELECT USING (bucket_id = 'client-portal-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for client-portal-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'client-portal-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for client-portal-files" ON storage.objects FOR UPDATE USING (bucket_id = 'client-portal-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for client-portal-files" ON storage.objects FOR DELETE USING (bucket_id = 'client-portal-files');`
    ];
