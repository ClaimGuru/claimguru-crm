-- Migration: enhance_crm_entity_management_system
-- Created at: 1755287495

-- Migration: Enhance CRM Entity Management System for Function 2.3
...
-- Create organization-based RLS policies for all tables
DO $$
DECLARE
    table_name TEXT;
    table_names TEXT[] := ARRAY[
        'vendor_categories', 'vendor_specialties', 'service_areas', 'vendor_service_areas',
        'vendor_equipment', 'vendor_performance_metrics', 'legal_specializations', 
        'attorney_profiles', 'attorney_case_assignments', 'referral_types',
        'referral_source_profiles', 'referral_conversions', 'claim_entity_associations',
        'entity_communications', 'entity_performance_summary'
    ];
BEGIN
    FOREACH table_name IN ARRAY table_names
    LOOP
        -- Allow access based on organization_id where it exists
        IF table_name IN ('vendor_service_areas', 'vendor_equipment', 'attorney_profiles', 'attorney_case_assignments', 'referral_conversions') THEN
            -- These tables don't have direct organization_id, handle differently
            CONTINUE;
        ELSE
            EXECUTE format('
                CREATE POLICY "Users can access %I from their organization" ON %I
...
-- Special RLS policies for tables without direct organization_id
...
FOR ALL USING (adjuster_id IN (
    SELECT id FROM adjusters WHERE organization_id IN (
        SELECT organization_id FROM user_profiles WHERE id = auth.uid()
    )
));
...
FOR ALL USING (referral_source_id IN (
    SELECT rsp.id FROM referral_source_profiles rsp
    JOIN lead_sources ls ON rsp.lead_source_id = ls.id
    WHERE ls.organization_id IN (
        SELECT organization_id FROM user_profiles WHERE id = auth.uid()
    )
));;