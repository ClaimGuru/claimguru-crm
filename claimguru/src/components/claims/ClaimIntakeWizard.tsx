...
const ClaimIntakeWizard: React.FC<ClaimIntakeWizardProps> = ({ onComplete, onCancel }) => {
  ...
  const updateIntakeData = (updates: Partial<ClaimIntakeData>) => {
    setIntakeData(prev => {
      ...
      // Auto-calculate estimated loss value based on contract fee type
      if (updates.contract_fee_type || updates.contract_fee_amount || updates.estimated_hours || updates.loss_amount) {
        const feeType = newData.contract_fee_type
        const feeAmount = newData.contract_fee_amount || 0
        const hours = newData.estimated_hours || 0
        const lossAmount = newData.loss_amount || 0
        
        let estimatedValue = 0
        
        switch (feeType) {
          case 'percentage':
            estimatedValue = lossAmount * (feeAmount / 100)
            break
          case 'hourly':
            estimatedValue = feeAmount * hours
            break
          case 'flat_fee':
        ...
        newData.estimated_loss_value = estimatedValue
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const claimData = {
        ...
        claim_status: 'open',
        claim_phase: 'intake',
        contract_fee_type: intakeData.contract_fee_type,
        contract_fee_amount: intakeData.contract_fee_amount
      }
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="border-t pt-6">
              <h4 className="text-sm font-medium text-gray-900 mb-4">Contract Fee Information</h4>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Contract Fee Type *</label>
                  <select
                    required
                    value={intakeData.contract_fee_type}
                    onChange={(e) => updateIntakeData({ contract_fee_type: e.target.value })}
                    className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Contract Fee Amount *
                    {intakeData.contract_fee_type === 'percentage' && ' (%)'}
                    {intakeData.contract_fee_type === 'hourly' && ' ($/hour)'}
                    {intakeData.contract_fee_type === 'flat_fee' && ' ($)'}
                  </label>
                  <input
                    type="number"
                    min="0"
                    step={intakeData.contract_fee_type === 'percentage' ? '0.1' : '0.01'}
                    required
                    value={intakeData.contract_fee_amount || ''}
                    onChange={(e) => updateIntakeData({ contract_fee_amount: parseFloat(e.target.value) })}
                    className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    placeholder={intakeData.contract_fee_type === 'percentage' ? '10' : '0.00'}
                  />
                </div>
                
                {intakeData.contract_fee_type === 'hourly' && (
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      step="0.5"
                      value={intakeData.estimated_hours || ''}
                      onChange={(e) => updateIntakeData({ estimated_hours: parseFloat(e.target.value) })}
                      className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      placeholder="0"
                    />
                  </div>
                ...
                {intakeData.contract_fee_type === 'percentage' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Loss Amount ($)</label>
                    <input
                      type="number"
                      min="0"
                      step="0.01"
                      value={intakeData.loss_amount || ''}
                      onChange={(e) => updateIntakeData({ loss_amount: parseFloat(e.target.value) })}
                      className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      placeholder="0.00"
                    />
                  </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mb-4">
                <div className="flex">
                  ...
                  <div>
                    <h5 className="text-sm font-medium text-blue-900">Fee Calculation</h5>
                    <p className="text-sm text-blue-700 mt-1">
                      {intakeData.contract_fee_type === 'percentage' && intakeData.contract_fee_amount && intakeData.loss_amount
                        ? `${intakeData.contract_fee_amount}% of $${intakeData.loss_amount.toLocaleString()} = $${((intakeData.loss_amount * intakeData.contract_fee_amount) / 100).toLocaleString()}`
                        : intakeData.contract_fee_type === 'hourly' && intakeData.contract_fee_amount && intakeData.estimated_hours
                        ? `${intakeData.estimated_hours} hours × $${intakeData.contract_fee_amount}/hour = $${(intakeData.estimated_hours * intakeData.contract_fee_amount).toLocaleString()}`
                        : intakeData.contract_fee_type === 'flat_fee' && intakeData.contract_fee_amount
                        ? `Flat fee: $${intakeData.contract_fee_amount.toLocaleString()}`
                        : 'Enter fee details to see calculation'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Estimated Loss Value (Calculated)</label>
                <input
                  type="number"
                  value={intakeData.estimated_loss_value || 0}
                  readOnly
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-50 text-gray-500 sm:text-sm"
                  placeholder="$0.00"
                />
                <p className="mt-1 text-xs text-gray-500">This value is automatically calculated based on your contract fee settings above.</p>
              </div>
              <div>
                      ...
                      .map((category) => {
                        ...
                        return (
                          <label key={`rec-${category.name}`} className={`flex items-center p-3 border rounded-lg cursor-pointer transition-colors ${
                            ...
                            />
                            <IconComponent className="ml-2 h-4 w-4 text-green-600" />
