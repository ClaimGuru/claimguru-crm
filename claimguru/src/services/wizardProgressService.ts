...
export class WizardProgressService {
    ...
    try {
      ...
      try {
        ...
        if (searchError && searchError.message?.includes('406')) {
          console.warn('🔄 Database RLS issue detected (406), switching to local storage fallback');
        ...
        if (existingProgress) {
          ...
          if (error) {
            ...
            if (error.message?.includes('406')) {
              console.warn('🔄 Database RLS issue on update (406), switching to local storage fallback');
              this.fallbackMode = true;
              return this.saveToLocalStorage(progress);
            }
          ...
          if (error) {
            // Handle 406 error specifically
            if (error.message?.includes('406')) {
              console.warn('🔄 Database RLS issue on insert (406), switching to local storage fallback');
              this.fallbackMode = true;
              return this.saveToLocalStorage(progress);
            }
        ...
        console.log('✅ Wizard progress saved successfully to database');
  ...
  private static saveToLocalStorage(progress: Omit<WizardProgress, 'id' | 'created_at'>): WizardProgress {
    try {
      ...
      console.log('✅ Wizard progress saved to local storage (fallback mode)');
      return progressData;
    } catch (error) {
  ...
  private static loadFromLocalStorage(userId: string, organizationId: string, wizardType: string): WizardProgress | null {
    try {
      ...
      if (progress) {
        ...
        console.log('✅ Loaded wizard progress from local storage');
      }
    ...
    try {
      ...
      try {
        ...
        if (error) {
          ...
          if (error.message?.includes('406')) {
            console.warn('🔄 Database RLS issue on load (406), switching to local storage fallback');
            this.fallbackMode = true;
            return this.loadFromLocalStorage(userId, organizationId, wizardType);
          }
        ...
        console.log('✅ Loaded wizard progress from database');
        return data;
  ...
  private static async manageReminderTask(progress: WizardProgress): Promise<void> {
    try {
      ...
      if (this.fallbackMode) {
        console.log('⏭️ Skipping reminder task creation (fallback mode)');
        return;
      }
  ...
  private static cleanupLocalStorage(): void {
    try {
      ...
      console.log('✅ Cleaned up expired local storage progress');
    } catch (error) {
  ...
  static async syncLocalStorageToDatabase(): Promise<boolean> {
    try {
      ...
      if (syncSuccessful) {
        ...
        this.fallbackMode = false;
        console.log('✅ Successfully synced local storage to database');
      }

      return syncSuccessful;
    } catch (error) {
      console.error('❌ Failed to sync local storage to database:', error);
      return false;
    }
  }
  ...
  static async checkAndSync(): Promise<void> {
    if (this.fallbackMode) {
      ...
      if (synced) {
        console.log('🔄 Database connection restored, synced local data');
      }
    }
  }
}