...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "image/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for claim-documents" ON storage.objects FOR SELECT USING (bucket_id = 'claim-documents');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for claim-documents" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'claim-documents');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for claim-documents" ON storage.objects FOR UPDATE USING (bucket_id = 'claim-documents');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for claim-documents" ON storage.objects FOR DELETE USING (bucket_id = 'claim-documents');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'claim-documents',
        public: true,
        allowed_mime_types: ["application/pdf", "image/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain"],
