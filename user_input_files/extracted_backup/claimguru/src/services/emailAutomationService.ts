    ...
    try {
      ...
      if (response.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          console.warn('Email configuration endpoint returned non-JSON response, using default config')
        }
      } else {
        console.warn('Email configuration endpoint not available, using default config')
      }
    } catch (error) {
      console.warn('Email configuration loading skipped:', error.message)
      // Continue with default configuration - don't let this block the app
    }
  }
